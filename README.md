...
アプリ内の以下の場所に自然なデザインのバナー広告用プレースホルダーを設置しています。
- **トップページ**（メインメニュー下部）
- **リザルト画面**（試合結果ダイアログのアクションボタン上部）
- **オンラインロビー画面**（部屋一覧の下部、およびルーム内チャット欄の下部）

### 設置を推奨するバナーサイズ
これらのプレースホルダーはレスポンシブ幅（`w-full`）かつ高さ `70px 〜 90px` 程度で設計されています。
実際の広告ネットワーク（Google AdSense等）を導入する際は、以下のサイズを推奨します。

- **レスポンシブ広告（フレキシブルバナー）** ※最も推奨
- **320 × 50** （モバイルリーダーボード - スマホ向け標準）
- **320 × 100** （ラージモバイルバナー - スマホ向け大きめ）
- **468 × 60** （標準バナー - タブレット/PC向け）
- **728 × 90** （ビッグバナー/リーダーボード - PC向け）
