...
paicrash オンライン対戦機能のデプロイ・インフラ・テスト・UI 拡張に関する実装タスク一覧。
...
- [x] 4. InfraStack 実装
  ...
  - ECR リポジトリ `paicrash-server` を実装する（`removalPolicy: RETAIN`）
...
- [x] 10. レポートダウンロード UI 実装
  ...
  - ファイル名を `paicrash-battle-{roomId}-{YYYYMMDD}.json` 形式とする
  - _Requirements: 8.1, 8.2, 8.3, 8.4, 8.5, 8.6_
  - _Depends on: 9_
