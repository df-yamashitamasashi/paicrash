...
本ドキュメントは、麻雀パズル落ちゲー「paicrash」のオンライン対戦機能に関する要件を定義する。
...
4. THE System SHALL レポートファイル名を `paicrash-battle-{roomId}-{YYYYMMDD}.json` の形式とする
